...
class ReviewService {
  ...
  createReview = async (
    ...
    try {
      ...
      console.log(doCityDong)
      if (administrativeDistrict.includes(doCityDong[0])) {
        await this.reviewRepository.createDo(doCityDong[0]);
        await this.reviewRepository.createCity(doCityDong[0]);
        await this.reviewRepository.createDong(doCityDong[2]);
      } else {
        await this.reviewRepository.createDo(doCityDong[0]);
        await this.reviewRepository.createCity(doCityDong[1]);
        await this.reviewRepository.createDong(doCityDong[2]);
      }
      ...
      if (reviewExist) {
        ...
        let estate = await this.reviewRepository.createEstate(address, address_jibun, latLng);
        let review = await this.reviewRepository.createReview(
          estate.estateId,
          // nickname,
          good,
